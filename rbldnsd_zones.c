...
static struct dataset *newdataset(char *spec) {
  ...
  *dsp = ds;
  ...
  /* MySQL datasets don't need files - they get config from ds_spec */
  if (strcmp(spec, "mysql") == 0) {
    /* For MySQL, don't parse files, just store the config as-is */
    ds->ds_dsf = NULL;
  } else {
    dsfp = &ds->ds_dsf;
...
int loaddataset(struct dataset *ds) {
  ...
  printf("DEBUG: loaddataset called for type: %s, spec: %s\n", ds->ds_type->dst_name, ds->ds_spec);
  fflush(stdout);
  ...
  /* Handle MySQL datasets without files */
  if (strcmp(ds->ds_type->dst_name, "mysql") == 0) {
    printf("DEBUG: Loading MySQL dataset: %s\n", ds->ds_spec);
    fflush(stdout);
    ds->ds_type->dst_startfn(ds);
    ds->ds_stamp = time(NULL);
