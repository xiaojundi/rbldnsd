...
break;
  ...
  /* Check if we should use MySQL auto-discovery */
  if (argc == 1 && strcmp(argv[0], "mysql") == 0) {
    /* Auto-discover zones from MySQL database */
    if (!ds_mysql_discover_zones(&zonelist)) {
      error(0, "failed to discover zones from MySQL database");
    }
  } else {
    /* Use command line zones */
    for(c = 0; c < argc; ++c)
      zonelist = addzone(zonelist, argv[c]);
  }
  init_zones_caches(zonelist);
...
static int do_reload(int do_fork) {
  ...
  if (!ds && call_hook(reload_check, (zonelist)) == 0) {
    ...
    for (zone = zonelist; zone; zone = zone->z_next) {
      ...
      if (has_mysql_datasets) break;
    }
    if (!has_mysql_datasets) {
      check_expires();
